...
export const parseCsv = (text: string): { rows: IImportRow[]; unknown: string[] } => {
  ...
  const rows = lines.slice(1).map((line) => {
    ...
    return {
      ...
      media: entry.media
        ? entry.media.split('|').map((item) => item.trim()).filter(Boolean)
        : undefined,
    };
...
export const buildTemplate = (): string => [
  ...
  'Футболка базовая;199;249;Женщинам;PHENOMEN;Хлопок 100%;piece;',
  'Сыр гауда;89;;Продукты;;Вес за килограмм;kg;',
].join('\n');
